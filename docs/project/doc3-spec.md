...
### 1.2 文档规范
- **Architect 设计**: `docs/project/architect/task-{编号}-{描述}.md`
- **Engineer 日志**: `docs/project/engineer/task-{编号}-{描述}.md`
- **模板**: 参考 `engineer/template.md`
- **强制要求**: 每个 Task 完成后必须创建对应文档
...
每个 Shortcut 的完整 XML 骨架设计（数据流、UUID 引用链、XML 模板、设计决策）已拆分为独立文件：

| 任务 | 设计文档 | Engineer 日志 | 状态 |
|------|----------|---------------|------|
...
以下是从实战中提炼的关键经验，适用于所有后续 Shortcut 设计：

- **text.replace 的 WFInput 必须用 WFTextTokenString**，不能用 WFTextTokenAttachment（详见 3C-1 §8）
- **OCR 文本含真实换行符**，嵌入 JSON body 前必须清洗控制字符（gettext → text.replace → downloadurl）
- **Shortcuts 索引从 1 开始**（WFItemIndex=1 对应第一项）
